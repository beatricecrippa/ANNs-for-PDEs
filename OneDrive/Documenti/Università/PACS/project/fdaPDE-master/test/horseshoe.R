...
sol1 <- smooth.FEM(observations = data1, FEMbasis = fembasis, location=locations,
                   lambda = lambda,lambda.selection.lossfunction = NULL, solver.options = "block_preconditioner")
...
points(log10(best_lambda1[1]), res5[10], pch=19, col='red')
...
for (i in 1:20)
{
  sol <- smooth.FEM(observations = data+rnorm(length(data), sd=0.1*(ran[2]-ran[1])),
...
for (i in 1:20)
{
  solCov <- smooth.FEM(locations=locations,observations = dataCov+rnorm(ndata, sd=0.1*(ranCov[2]-ranCov[1])),
                       FEMbasis = fembasis, covariates=cbind(cov1_exactCov,cov2_exactCov),
                      lambda.selection.criterion = 'newton', DOF.evaluation = 'exact')#,
...
###### execution times #####
tB=microbenchmark(smooth.FEM(observations = data, FEMbasis = fembasis, locations=locations,lambda=best_lambda1[1], solver.options = "block_preconditioner"))
