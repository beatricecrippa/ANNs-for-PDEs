...
for (i in 1:20)
{
  sol <- smooth.FEM(observations = data+rnorm(length(data)[1],sd=0.1*(ran[2]-ran[1])),FEMbasis = FEMbasis,
                    lambda.selection.criterion = 'newton', DOF.evaluation = 'exact',
                    lambda.selection.lossfunction = 'GCV',
                    solver.options="block_preconditioner")
  sol_approx <- sol$fit.FEM
  best_lambda4 <- c(best_lambda4,sol$optimization$lambda_solution)
...
boxplot(rmseE[which(rmseE>1.8)],rmseF[which(rmseF>1.8)],rmseG[which(rmseG>1.8)],rmseH[which(rmseH>1.8)], names=c('fdaPDE', 'mass lumping', 'lambda','block'), col=c('grey',2,3,4))
...
solCov <- smooth.FEM(observations = dataCov,
                     ...
                     lambda.selection.criterion = 'newton', DOF.evaluation = 'exact')
best_lambdaCov6 <- solCov$optimization$lambda_solution

RMSE<-function(f,g) sqrt(mean((f-g)^2))
rmseCov =NULL
for (i in 1:20)
{
  solCov <- smooth.FEM(observations = dataCov+ rnorm (nnodes, sd=0.1*(ran[2]-ran[1])),
                       FEMbasis = FEMbasis, covariates=cbind(cov1,cov2),
