...
for(l in lambda)
{
  ...
  for (i in 1:30)
  {
    output_CPP<-smooth.FEM(observations=data+rnorm (length(nodes[1,]), sd=0.1*(ran[2]-ran[1])),
                           FEMbasis=FEMbasis, solver.options="block_preconditioner", #locations=locations, #covariates=cov,
                           lambda=l)
...
for(l in lambda)
{
  ...
  for (i in 1:30)
  {
    output_CPP<-smooth.FEM(observations=dataCov+rnorm (length(nodes[1,]), sd=0.1*(ranCov[2]-ranCov[1])),
                           FEMbasis=FEMbasis, covariates=cov, solver.options = "block_preconditioner",#locations=locations,
                           lambda=l)
    # best_lambda<-c(best_lambda,
    #               which(output_CPP$optimization$GCV_vector==min(output_CPP$optimization$GCV_vector)))
    # output_CPP<-smooth.FEM(observations=data3d+rnorm (length(nodes3d[1,]), sd=0.01*(ran3d[2]-ran3d[1])),
...
for(l in lambda)
{
  ...
  for (i in 1:30)
  {
    output_CPP<-smooth.FEM(locations = loc, observations = as.numeric(rpois(length(loc[,1]), lambda = mu4)),
                           FEMbasis =FEMbasis, covariates = NULL, solver.options = "block_preconditioner",
...
plot(log10(lambda), colMeans(rmseA), pch=4, col=1, ylab="Mean RMSE")#, ylim=c(min(colMeans(rmse4A)),max(colMeans(rmse4B))))
...
plot(log10(lambda), colMeans(rmse2A), pch=4, col=1, ylab="Mean RMSE")
...
boxplot(rmseGLMA[,which.min(colMeans(rmse4A))],rmseGLMB[,which.min(colMeans(rmseGLMB))],
