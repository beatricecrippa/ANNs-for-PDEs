...
sol_approxnoCov <- solnoCov$fit.FEM
best_lambdanoCov6 <- solnoCov$optimization$lambda_solution
RMSE<-function(f,g) sqrt(mean((f-g)^2))
loc_eval = rsphere(5000)
rmsenoCov =NULL
for (i in 1:20)
{
  solnoCov <- smooth.FEM(observations = dataCov+rnorm(nrow(mesh$nodes), mean=0, sd=0.1*diff(range(coeffCov))),
                         FEMbasis = FEMbasis,
                         lambda.selection.criterion = 'newton',
                         DOF.evaluation = 'exact', #solver.options = "block_preconditioner",
                         lambda.selection.lossfunction = 'GCV')
  sol_approxnoCov <- solnoCov$fit.FEM
  best_lambdanoCov2 <- c(best_lambdanoCov2,solnoCov$optimization$lambda_solution)
  rmsenoCov <- c(rmsenoCov, RMSE(func(loc_eval),
                             eval.FEM(solnoCov$fit.FEM,loc_eval)))
}
boxplot(rmsenoCov)
rmsenoCovB = rmsenoCov

boxplot(rmsenoCovA,rmsenoCovB,rmsenoCovC,rmsenoCovD,names=c('fdaPDE','mass lumping','lambda','block'),col=c('grey',2,3,4),ylab="RMSE")
...
for (i in 1:20)
{
  solCov <- smooth.FEM(observations = dataCov+beta[1]*cov1+beta[2]*cov2+rnorm(nrow(mesh$nodes), mean=0, sd=0.1*diff(range(coeffCov))), covariates = W,
                       FEMbasis = FEMbasis,
                       lambda.selection.criterion = 'newton',
                       DOF.evaluation = 'exact',  #solver.options = "block_preconditioner",
                       lambda.selection.lossfunction = 'GCV')
...
best_lambda6 <- sol$optimization$lambda_solution
RMSE<-function(f,g) sqrt(mean((f-g)^2))
