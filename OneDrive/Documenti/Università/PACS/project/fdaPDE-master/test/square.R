...
ran = range(sol_exact)
data = sol_exact
data1 = sol_exact + rnorm(nnodes, mean=0, sd=0.001*abs(ran[2]-ran[1]))

# Set smoothing parameter
lambda = 10^(-12:3)

#### Test 1.1: Without GCV
output_CPP<-smooth.FEM(observations=data, FEMbasis=FEMbasis, lambda=lambda, solver.options="block_preconditioner")
#image(output_CPP$fit.FEM)
sol_approx = output_CPP$fit.FEM
femsol_approx<-t(eval.FEM(sol_approx,nodes))

output_CPP1<-smooth.FEM(observations=data1, FEMbasis=FEMbasis, lambda=lambda, solver.options="block_preconditioner")
...
output_CPP<-smooth.FEM(observations=data, FEMbasis=FEMbasis, lambda.selection.criterion='newton', DOF.evaluation='exact', lambda.selection.lossfunction='GCV', solver.options = "mass_lumping")
best_lambda6<-output_CPP$optimization$lambda_solution
image(FEM(output_CPP$fit.FEM$coeff,FEMbasis))
...
legend('topright',legend=c('no preconditioner', 'mass lumping', 'lambda preconditioner', 'block_preconditioner'), cex=0.62,col=1:4, pch=19)
...
for (i in 1:20)
{
  output_CPP<-smooth.FEM(observations=data+rnorm(nnodes, mean=0, sd=0.1*abs(ran[2]-ran[1])),
                         ...
                         DOF.evaluation='exact', solver.options="block_preconditioner",
                         lambda.selection.lossfunction='GCV')
  best_lambda4<-c(best_lambda4,output_CPP$optimization$lambda_solution)
  sol_approx=output_CPP$fit.FEM
  rmse <- c(rmse, RMSE(zCov(cbind(xeval,yeval)),eval.FEM(sol_approx,locations=cbind(xeval,yeval))))
}
...
data2 = sol_exact2
data12 = sol_exact2 + rnorm(nnodes2, mean=0, sd=0.001*abs(ran2[2]-ran2[1]))
...
points(log10(best_lambdaCov1[1]), resCov5[13], pch=19, col='red')
...
for (i in 1:20)
{
  output_CPP<-smooth.FEM(observations=dataCov+rnorm(nnodes, mean=0, sd=0.1*abs(ranCov[2]-ranCov[1])),
                         FEMbasis=FEMbasis, covariates=cov,
                         lambda.selection.criterion='newton', solver.options="block_preconditioner",
                         DOF.evaluation='exact', lambda.selection.lossfunction='GCV')
  best_lambdaCov4<-c(best_lambdaCov4,output_CPP$optimization$lambda_solution)
...
###### execution times #####
tA=microbenchmark(smooth.FEM(observations=data, FEMbasis=FEMbasis, lambda=best_lambda1[1]),times=30)
t2D=microbenchmark(smooth.FEM(observations=data2, FEMbasis=FEMbasis2, lambda=best_lambda21))
tcovB=microbenchmark(smooth.FEM(observations=dataCov, FEMbasis=FEMbasis,
                                covariates=cov, lambda=best_lambdaCov1[1]),times=30)
